...
<form on:submit={handleSubmit} class="needs-validation">
	{#each fields as field}
		<div class="mb-3">
			{#if field.showLabel}
				<label for={field.id} class="form-label">{field.label || field.name}</label>
			{/if}
			...
			{:else if field.type === 'checkbox'}
				<div class="form-check">
					<input
						...
						required={field.required || false}
					/>
					<label class="form-check-label" for={field.id}>{field.label || field.name}</label>
				</div>
	...
	<div class="text-end">
		<button disabled={props.disabled || false} type="submit" class="btn {props.submitClass}">
