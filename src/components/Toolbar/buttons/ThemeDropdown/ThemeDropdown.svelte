...
<div class="example-dropdown dd-wrapper d-flex align-items-center dd pointer mt-1">
	<div class="dropdown dropend">
		<button class="btn btn-toolbar" type="button" on:click={toggleDropdown} aria-expanded={isOpen}>
			<i class="bi bi-image"></i>
		</button>
		<div class="dropdown-menu {isOpen ? 'show' : ''}" bind:this={dropdownMenu} aria-labelledby="dropdownMenuButton">
			<div class="grid-container">
				{#each swatches as swatch}
					<div
						class="grid-item {swatch.bgImage === $_app.view.theme.bgImage ? 'selected' : ''}"
				...
				<!-- {#each colors as color}
					<div
						class="grid-item {color === $_app.view.theme.color ? 'selected-color' : ''}"
