<script>
  ...
  $: depth =
    $_app.view.mode == "cards" ? 0 : card?.dependencyInfo.parentCardDepth * 10;

  // $: console.log(card);
</script>

<NodeViewWrapper {...node.attrs} id={node.attrs.id} class="fable-card">
  <div class="w-100 fable-card-inner p-3 rounded">
    <NodeViewContent />
  </div>
  {#if card?.pages?.display}
    <div contenteditable="false" class="page-count text-end">
      {card?.pages?.display}.
    </div>
  {/if}
</NodeViewWrapper>
